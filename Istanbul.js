...
axios.get('https://ais.usvisa-info.com/en-tr/niv/users/sign_in', {
  ...
  withCredentials: true
    ...
    axios.get('https://ais.usvisa-info.com/en-tr/niv/schedule/49489128/appointment/days/125.json?appointments[expedite]=false', {
        headers: {
  ...
  .then(response => {
    ...
    if (earlierDates.length > 0) {
      console.log('Earlier date(s) found for Istanbul Visa, Sending Email! Claim them as soon as possible!:'.green, earlierDates.join(', ').green);
      ...
      var mailOptions = {
        ...
        text: 'Earlier date(s) found for Istanbul Visa, Sending Email! Claim them as soon as possible!: THIS IS A TEST ' + earlierDates.join(', ')
      };
      transporter.sendMail(mailOptions, function(error, info){
        if (error) {
          console.log(error);
        } else {
          console.log('Istanbul Dates Email Sent!'.green);
        }
      });
    } else {
      console.log('Earlier date not found for Istanbul Visa, try again later!'.red);
    }
